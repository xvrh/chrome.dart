...
import 'package:chrome/gen/tabs.dart' as chrome;
...
void main() {
  group('chrome.tabs', () {
    ...
    setUp(() {
      chrome.WindowsCreateParams createData = new chrome.WindowsCreateParams(
    ...
    test('highlight', () {
      chrome.TabsHighlightParams highlightInfo = new chrome.TabsHighlightParams(
    ...
    test('update', () {
      chrome.TabsUpdateParams updateProperties = new chrome.TabsUpdateParams(
          url: 'http://www.google.com/',
          active: true,
          highlighted: true,
          pinned: true);
...
//              expect(movedTabs.first.index, 1);
    ...
    test('move 2 tabs', () {
      ...
      chrome.tabs
          ...
          .then(
              (_) => chrome.tabs.move([newTab1.id, newTab2.id], moveProperties))
          .then(expectAsync((List<chrome.Tab> movedTabs) {
        expect(movedTabs, isNotNull);
...
//              expect(movedTabs[1].index, anyOf(1, 2));
    ...
    test('reload', () {
      chrome.tabs.reload(window.tabs.first.id).then(expectAsync((_) {}));
    });
    ...
    test('detectLanguage', () {
      chrome.tabs
          .detectLanguage(window.tabs.first.id)
