...
class ChromeDevtools {
  ...
  /**
   * Accessor for the `chrome.devtools.recorder` namespace.
   */
  final ChromeDevtoolsRecorder recorder = ChromeDevtoolsRecorder._();
}
 ...
 * DevTools.
 */
class ChromeDevtoolsRecorder extends ChromeApi {
  JsObject get _devtools_recorder => chrome['devtools']['recorder'];

  ChromeDevtoolsRecorder._();

  bool get available => _devtools_recorder != null;
   ...
   * 
   * [mediaType] The media type of the string content that the plugin produces.
   */
  void registerRecorderExtensionPlugin(RecorderExtensionPlugin plugin, String name, String mediaType) {
    if (_devtools_recorder == null) _throwNotAvailable();

    _devtools_recorder.callMethod('registerRecorderExtensionPlugin', [jsify(plugin), name, mediaType]);
  }
   ...
   * directory.
   */
  RecorderView createView(String title, String pagePath) {
    if (_devtools_recorder == null) _throwNotAvailable();

    return _createRecorderView(_devtools_recorder.callMethod('createView', [title, pagePath]));
  }

  void _throwNotAvailable() {
    throw  UnsupportedError("'chrome.devtools.recorder' is not available");
  }
}
 ...
 * panel.
 */
class RecorderExtensionPlugin extends ChromeObject {
  RecorderExtensionPlugin();
  RecorderExtensionPlugin.fromProxy(JsObject jsProxy): super.fromProxy(jsProxy);

  /**
   * Converts a recording from the Recorder panel format into a string.
   * 
   * [recording] A recording of the user interaction with the page. This should
   * match <a
   * href="https://github.com/puppeteer/replay/blob/main/docs/api/interfaces/Schema.UserFlow.md">Puppeteer's
   * recording schema</a>.
   */
  void stringify(DevtoolsRecorderStringifyParams recording) {
    jsProxy.callMethod('stringify', [jsify(recording)]);
  }
   ...
   * step schema</a>.
   */
  void stringifyStep(DevtoolsRecorderStringifyStepParams step) {
    jsProxy.callMethod('stringifyStep', [jsify(step)]);
  }
   ...
   * recording schema</a>.
   */
  void replay(DevtoolsRecorderReplayParams recording) {
    jsProxy.callMethod('replay', [jsify(recording)]);
  }
}
 ...
 * panel.
 */
class RecorderView extends ChromeObject {
  RecorderView();
  RecorderView.fromProxy(JsObject jsProxy): super.fromProxy(jsProxy);
...
class DevtoolsRecorderStringifyParams extends ChromeObject {
  DevtoolsRecorderStringifyParams();
  DevtoolsRecorderStringifyParams.fromProxy(JsObject jsProxy): super.fromProxy(jsProxy);
}

class DevtoolsRecorderStringifyStepParams extends ChromeObject {
  DevtoolsRecorderStringifyStepParams();
  DevtoolsRecorderStringifyStepParams.fromProxy(JsObject jsProxy): super.fromProxy(jsProxy);
}

class DevtoolsRecorderReplayParams extends ChromeObject {
  DevtoolsRecorderReplayParams();
  DevtoolsRecorderReplayParams.fromProxy(JsObject jsProxy): super.fromProxy(jsProxy);
}

RecorderView _createRecorderView(JsObject jsProxy) => RecorderView.fromProxy(jsProxy);
